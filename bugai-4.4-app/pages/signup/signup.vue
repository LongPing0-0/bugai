<template>
	<view class="content" :style="{height:screenHeight+'px'}">
		...
		<view class="content2">
			<input class="in" @input="checkInput" placeholder="用户名" v-model="signForm.username"
				onkeypress="javascript:if(event.keyCode == 32)event.returnValue = false;">
			<input class="in" password="true" placeholder="密码" v-model="signForm.password"
				onkeypress="javascript:if(event.keyCode == 32)event.returnValue = false;">
			<input class="in" placeholder="邮箱号" v-model="signForm.email"
				onkeypress="javascript:if(event.keyCode == 32)event.returnValue = false;">
			<input class="in" placeholder="验证码" v-model="signForm.code"
				onkeypress="javascript:if(event.keyCode == 32)event.returnValue = false;">
			<button class="bt1" :disabled="disabled" @click="getEmailCode()">{{codeMessage}}</button>
			<button class="bt2" @click="loginCheck()">注册账号</button>
		...
		methods: {
			checkInput(e) {
				...
				this.$nextTick(function() {
					this.signForm.username = o.value.replace(inputRule, '');
				})
			...
			loginCheck() {
				...
				if (
					...
					this.signForm.username == null || this.signForm.username == "" || this.signForm.password == null ||
				...
				else if(this.signForm.username.length <= 6){
				...
				else if(this.signForm.password.length <= 6){
					uni.showToast({
						title: "密码长度最少7位",
						icon:'none',
						duration: 1500
