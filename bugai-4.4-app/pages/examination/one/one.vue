	...
	export default {
		...
		onLoad(options) {
			...
			for (var i = 0; i < this.subjectList.length; i++) {
				this.$set(this.subjectList[i], "showAnswer", this.subjectList[i].showAnswer);
			}
		},
		mounted() {
			this.initSubject()
		...
		methods: {
			...
			initSubject(){
				if(this.param!=null){
					...
					this.request({
						...
						console.log(resp)
						if(resp.code == 200){
							this.subjectList = resp.data.subjectList
						}
					})
					// this.subjectList = this.param.subjectList
				}
			},
			//隐藏对话框
			hideModal(e) {
			...
			mySubmit(){
				...
				console.log(JSON.stringify(this.subjectList[0]))
				for (var i = 0; i < this.subjectList.length; i++) {
					console.log("输出长度")
